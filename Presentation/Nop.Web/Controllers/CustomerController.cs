...
namespace Nop.Web.Controllers
{
    ...
    public partial class CustomerController : BasePublicController
    {
        ...
        private readonly IKhKhachHangModelFactory _khachhangModelFactory;
        private readonly IKhKhachHangService _khachhangService;
        ...
        public CustomerController(AddressSettings addressSettings,
            ...
            IKhKhachHangModelFactory khachhangModelFactory,
            IKhKhachHangService khachhangService,
            ...
            IKhThongTinMoRongService khThongTinMoRongService)
        {
            ...
            _khachhangModelFactory = khachhangModelFactory;
            _khachhangService = khachhangService;
            _khDanhBaDienThoaiService = khDanhBaDienThoaiService;
            _khThongTinMoRongService = khThongTinMoRongService;
        }
        ...
        #region Methods
        #region RegisterCRMKH
        [HttpsRequirement]
        //available even when navigation is not allowed
        [CheckAccessPublicStore(true)]
        public virtual IActionResult RegisterCRMKH()
        {
            //check whether registration is allowed
            if (_customerSettings.UserRegistrationType == UserRegistrationType.Disabled)
                return RedirectToRoute("RegisterResult", new { resultId = (int)UserRegistrationType.Disabled });
            var model = _khachhangModelFactory.PrepareKhKhachHangModel(new KhKhachHangModel(), null);
            model.DDLGioiTinh.First(d => d.Text.Equals("Tất cả")).Text = "Giới tính";
            return View(model);
        }
        [HttpPost]
        [ValidateCaptcha]
        ...
        //available even when navigation is not allowed
        [CheckAccessPublicStore(true)]
        public virtual IActionResult RegisterCRMKH(KhKhachHangModel model, string returnUrl, bool captchaValid, IFormCollection form)
        {
            //check whether registration is allowed
            if (_customerSettings.UserRegistrationType == UserRegistrationType.Disabled)
                return RedirectToRoute("RegisterResult", new { resultId = (int)UserRegistrationType.Disabled });
            ...
            if (ModelState.IsValid)
            {
                if (string.IsNullOrEmpty(model.SoDienThoai))
                {
                    ...
                    //var x = new { SoDienThoai = new { errors = new string[1] { "Vui lòng nhập số điện thoại" } } };
                    return this.JsonErrorMessage("Có lỗi !", ModelState.SerializeErrors());
                }
                if (!string.IsNullOrWhiteSpace(model.SoDienThoai) && _khDanhBaDienThoaiService.GetKhDanhBaDienThoaiBySoDienThoai(model.SoDienThoai, _storeContext.CurrentStore.Id) != null)
                {
                    ModelState.AddModelError("SoDienThoai", "Số điện thoại này đã được đăng kí trong hệ thống!!");
                    return this.JsonErrorMessage("Có lỗi !", ModelState.SerializeErrors());
                }
                var item = model.ToEntity<KhKhachHang>();
                item.DOANH_NGHIEP_ID = _storeContext.CurrentStore.Id;
                _khachhangService.InsertKhKhachHang(item);
                // danh ba dien thoai
            ...
            if (ModelState.IsValid)
            {
                ...
                switch (loginResult)
                {
                    ...
                    case CustomerLoginResults.CustomerNotExist:
                        retMsg = _localizationService.GetResource("Account.Login.WrongCredentials.CustomerNotExist");
                        break;
                    case CustomerLoginResults.Deleted:
                        retMsg = _localizationService.GetResource("Account.Login.WrongCredentials.Deleted");
                        break;
                    case CustomerLoginResults.NotActive:
                        retMsg = _localizationService.GetResource("Account.Login.WrongCredentials.NotActive");
                        break;
                    case CustomerLoginResults.NotRegistered:
                        retMsg = _localizationService.GetResource("Account.Login.WrongCredentials.NotRegistered");
                        break;
                    case CustomerLoginResults.LockedOut:
                        retMsg = _localizationService.GetResource("Account.Login.WrongCredentials.LockedOut");
                        break;
                    case CustomerLoginResults.WrongPassword:
                    default:
                        retMsg = _localizationService.GetResource("Account.Login.WrongCredentials");
                        break;
                }
