...
namespace Nop.Services.Customers
{
    ...
    public partial class CustomerService : ICustomerService
    {
        ...
        public virtual IPagedList<Customer> GetAllCustomers(DateTime? createdFromUtc = null, DateTime? createdToUtc = null,
            ...
            int pageIndex = 0, int pageSize = int.MaxValue, bool getOnlyTotalCount = false, int idStore = 0)
        {
            ...
            if (idStore > 0)
                query = query.Where(c => idStore == c.RegisteredInStoreId);
        ...
        public virtual Customer GetCustomerByNhanVienId(int nhanVienId)
        {
            if (nhanVienId == 0)
                return null;

            return _customerRepository.Table.FirstOrDefault(c => c.NhanVienId == nhanVienId);
        }
        ...
        public virtual IList<CustomerRole> GetAllTBOficesCustomerRoles(string Module)
        {
            Module = "TBOffices" + Module;
            var key = _cacheKeyService.PrepareKeyForShortTermCache(NopCustomerServicesDefaults.CustomerRolesAllTBOfficesCacheKey, Module);
            return _staticCacheManager.Get(key, () =>
            {
                var query = from cr in _customerRoleRepository.Table
                            orderby cr.Name
                            where cr.Active && cr.SystemName.Contains(Module)
                            select cr;
