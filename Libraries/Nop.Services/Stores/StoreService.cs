...
namespace Nop.Services.Stores
{
    ...
    public partial class StoreService : IStoreService
    {
        ...
        public virtual Store GetStoreByKey(string AccessKey)
        {
            if (string.IsNullOrEmpty(AccessKey))
                return null; 
            return _storeRepository.Table.FirstOrDefault(c => c.AccessKey == AccessKey);
        }    
        ...
        public virtual void InsertStore(Store store)
        {
            if (store == null)
                throw new ArgumentNullException(nameof(store));
            string _accessKey = CommonHelper.GenerateRandomDigitCode(8);
            while(_storeRepository.Table.Any(c=>c.AccessKey==_accessKey))
            {
                _accessKey = CommonHelper.GenerateRandomDigitCode(8);
            }
            store.AccessKey = _accessKey;
