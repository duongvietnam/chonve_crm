...
namespace Nop.Services.Logging
{
    ...
    public class CustomerActivityService : ICustomerActivityService
    {
        ...
        public virtual ActivityLog InsertActivity(Customer customer, string systemKeyword, string comment, BaseEntity entity = null)
        {
            ...
            if (activityLogType == null)
            {
                var itemLogType = new ActivityLogType();
                itemLogType.Enabled = true;
                itemLogType.Name = systemKeyword;
                itemLogType.SystemKeyword = systemKeyword;
                InsertActivityType(itemLogType);

                //load lai
                activityLogType = GetAllActivityTypes().FirstOrDefault(type => type.SystemKeyword.Equals(systemKeyword));
            }
